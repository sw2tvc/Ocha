...
function StatCard({ label, value, sub, icon: Icon, accent }: {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center", accent ? "bg-white/20" : "bg-primary/10")}>
        <Icon size={16} className={accent ? "text-white" : "text-primary"} />
      </div>
      <div>
        <p className={cn("text-2xl font-bold", accent ? "text-white" : "text-foreground")}>{value}</p>
        <p className={cn("text-xs font-medium mt-0.5", accent ? "text-white/70" : "text-muted-foreground")}>{label}</p>
        {sub && <p className={cn("text-xs mt-0.5", accent ? "text-white/50" : "text-muted-foreground/70")}>{sub}</p>}
      </div>
    </div>
  );
}
...
export default function CleanerEarnings() {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-8 bg-background">
      {/* Header */}
      <div className="bg-primary text-primary-foreground px-6 pt-8 pb-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white/10 rounded-2xl p-6 md:inline-block">
            <p className="text-white/60 text-xs font-medium mb-1">This year</p>
            {isLoading ? (
              <div className="h-10 w-36 rounded-xl bg-white/20 animate-pulse" />
            ) : (
              <p className="text-5xl font-bold text-white">£{summary.thisYearEarnings ?? 0}</p>
            )}
            <p className="text-white/50 text-sm mt-1">{summary.totalJobs ?? 0} completed jobs</p>
          </div>
        </div>
      </div>

      <div className="max-w-6xl mx-auto w-full px-6 -mt-4 flex flex-col gap-6">

        {/* Summary stats — 4 cols on desktop */}
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard label="Avg per job"  value={`£${summary.avgPerJob ?? 0}`}  icon={Briefcase} accent />
            <StatCard label="Best week"    value={`£${summary.bestWeek ?? 0}`}   icon={Zap} />
            <StatCard label="Avg weekly"   value={`£${summary.avgWeekly ?? 0}`}  icon={TrendingUp} />
            <StatCard label="Total jobs"   value={String(summary.totalJobs ?? 0)} sub="all time" icon={Star} />
          </div>
        )}

        {/* Chart + breakdown — side by side on desktop */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Earnings chart — takes 2/3 on desktop */}
          <div className="md:col-span-2 bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm font-bold text-foreground uppercase tracking-wide">Earnings Breakdown</p>
              ...
              <ResponsiveContainer width="100%" height={260}>
                {view === "weekly" ? (
                  <BarChart data={chartData} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                    ...
                    <Bar dataKey="earnings" radius={[6, 6, 0, 0]} fill="hsl(var(--primary))" maxBarSize={36} />
                  </BarChart>
                ) : (
          ...
          {/* By service type — takes 1/3 on desktop */}
          {!isLoading && recentJobs.length > 0 && (() => {
            ...
            return (
              <div className="bg-card border border-border rounded-2xl p-5">
                <p className="text-sm font-bold text-foreground uppercase tracking-wide mb-4">By Service Type</p>
        ...
        {!isLoading && recentJobs.length > 0 && (
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-border">
              <p className="text-sm font-bold text-foreground">Recent Jobs</p>
            </div>
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground hidden lg:table-cell">Date</th>
                  <th className="text-right px-6 py-3 text-xs font-semibold text-muted-foreground">Earnings</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <p className="text-sm font-bold text-primary">£{job.totalPrice}</p>
                      </td>
                      <td className="px-6 py-4">
